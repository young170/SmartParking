...
def on_data_message(client, userdata, message, distance, cam_on_flag, car_in_flag, car_count):
    payload = message.payload.decode("utf-8")
...
def run2(url, mqtt_broker, mqtt_topic_car, car_count, cam_on_flag, published_time, car_in_flag, distance):
    ...
    client = mqtt.Client()
    client.on_message = lambda client, userdata, message: on_message(client, userdata, message, car_count)
    client.connect(mqtt_broker)
    client.subscribe(mqtt_topic_car)
    client.loop_start()
...
def run3(mqtt_broker, mqtt_topic_data, distance, cam_on_flag, car_in_flag, car_count):
    client = mqtt.Client()
    client.on_message = lambda client, userdata, message: on_data_message(client, userdata, message, distance, cam_on_flag, car_in_flag, car_count)
    client.connect(mqtt_broker)
    client.subscribe(mqtt_topic_data)
    client.loop_start()

    while True:
        time.sleep(1)  # Your data processing logic here
...
if __name__ == '__main__':
    ...
    process3 = multiprocessing.Process(target=run3, args=(mqtt_broker, mqtt_topic_data, distance, cam_on_flag, car_in_flag, car_count))
